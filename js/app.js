...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('ai-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            ...
            const savedQuiz = localStorage.getItem('ai-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('ai-progress', JSON.stringify(this.progress));
        localStorage.setItem('ai-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>AIアカデミー へようこそ</h2>
                    <p>人工知能の本質を理解し、ビジネスに活かす力を身につけましょう。機械学習からLLM、エージェントまで体系的に学びます。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('ai-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('ai-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('ai-progress');
            localStorage.removeItem('ai-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
